...
//16通道数据采集板扩展口定义（输出扩展）
/*
  16通道数据采集板扩展口线分配: 总线地址 = 0x6400 0000
	D0  - OUT1
	D1  - OUT2
	D2  - OUT3
	D3  - OUT4
	...
	D31  - 未引出				
*/
//16通道数据采集板扩展口线分配: 总线地址 = 0x6400 0000（输入扩展）
/*
	D0  - X1_1 输入端口1
	D1  - X1_2 输入端口2
	D2  - X1_3 输入端口3
...
void bsp_InitExtIO(void)
{
	...
	/* 将V6开发板一些片选，LED口设置为高 */
	g_HC574 = (NRF24L01_CE | NRF905_TRX_CE | LED1 | LED2 | LED3 | LED4);
...
void bsp_SetY(uint8_t _id, uint8_t _state)
{
	if (_id <= 7)
	{
		HC574_SetPin((1 << _id), _state);
	}
	else
	{
		HC574_SetPin((1 << (_id + 8)), _state);
	}
}
