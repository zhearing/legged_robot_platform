...
/*
	...
	//PC6/TIM3_CH1/TIM8_CH1     ----> AD7606_CONVST  (和摄像头复用),  输出PWM方波，作为ADC启动信号 V6
	PF8/TIM13_CH1             ----> AD7606_CONVST  (和摄像头复用),  输出PWM方波，作为ADC启动信号 16通道数据采集板
	//PE5/DCMI_D6/AD7606_BUSY   <---- AD7606_BUSY    , CPU在BUSY中断服务程序中读取采集结果 v6
	PC13/AD7606_BUSY            <---- AD7606_BUSY    , CPU在BUSY中断服务程序中读取采集结果 16通道数据采集板
...
/* 启动AD转换的GPIO : PC6 */
//#define CONVST_1()	GPIOC->BSRRL = GPIO_Pin_6  //V6
//#define CONVST_0()	GPIOC->BSRRH = GPIO_Pin_6
#define CONVST_1()	GPIOF->BSRRL = GPIO_Pin_8    //16通道数据采集板
#define CONVST_0()	GPIOF->BSRRH = GPIO_Pin_8
...
#define AD7606_RESULT()	*(__IO uint32_t *)0x6c000000   //16通道数据采集板
...
*	功能说明: 读取16路采样结果。结果存储在全局变量 g_tAD7606
*	形    参: 无
*	返 回 值: 无
*********************************************************************************************************
*/
void AD7606_ReadNowAdc(void)
{
	...
	g_tAD7606.sNowAdc[0] = AD7606_RESULT(); /* 读第1,9路样本 */
	g_tAD7606.sNowAdc[1] = AD7606_RESULT(); /* 读第2,10路样本 */
	g_tAD7606.sNowAdc[2] = AD7606_RESULT(); /* 读第3,11路样本 */
	g_tAD7606.sNowAdc[3] = AD7606_RESULT(); /* 读第4,12路样本 */
	g_tAD7606.sNowAdc[4] = AD7606_RESULT();	/* 读第5,13路样本 */
	g_tAD7606.sNowAdc[5] = AD7606_RESULT();	/* 读第6,14路样本 */
	g_tAD7606.sNowAdc[6] = AD7606_RESULT();	/* 读第7,15路样本 */
	g_tAD7606.sNowAdc[7] = AD7606_RESULT();	/* 读第8,16路样本 */
...
{
#if 1
	//配置PC6为TIM1_CH1功能，输出占空比50%的方波 V5	
	//bsp_SetTIMOutPWM(GPIOC, GPIO_Pin_6, TIM8, 1, _ulFreq, 5000);V5
	  bsp_SetTIMOutPWM(GPIOF, GPIO_Pin_8, TIM13, 1, _ulFreq, 5000);//16通道数据采集板
	#else	
	...
	/* 配置PE5, BUSY 作为中断输入口，下降沿触发 V6 */
	/* 配置PC13, BUSY 作为中断输入口，下降沿触发 16通道数据采集板 */
	{
...
*********************************************************************************************************
*/
uint8_t AD7606_ReadFifo(uint32_t *_usReadAdc)
{
